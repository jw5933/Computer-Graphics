...
*/
      ...
      // FIND A REASONABLE INITIAL VALUE FOR X:
         ...
         if (xx < yy && xx < zz) 
            ...
            X = [0,1,0]; //if there isn't much direction in x y or z, we can get an angle that is not aligned
         else (zz < xx && zz < yy) 
            X = [0,0,1];
...
`,
render: `
   S.revolutionMesh = S.createRevolutionMesh(64, 128, [
      {z:-1  , r:0  },
      {z:-.99, r:.1 },
      {z:-.7 , r:.5 },
      {z: .3 , r:.1 },
   ...
   S.extrusionMesh = S.createExtrusionMesh(64, 64, extrusionData);
   ...
   m.save();
      ...
      m.roty(time);
      // m.rotx(-Math.PI/2);
      m.rotz(Math.PI/8 * Math.sin(time));
