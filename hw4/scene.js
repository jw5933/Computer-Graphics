...
//pelvis
m.save();
m.translate(0, Math.cos(T)*0.15, 0);
   m.roty(Math.PI/4);
   m.roty(T- Math.PI);
   m.rotx(Math.sin(T-2)*0.1 + 0.1);
   ...
   //legs
      //hip left
      m.save();
         m.translate(-1 * PL * 0.55, -0.1, 0);
         m.rotx(-Math.sin(T-2)*0.1 - 0.1);
         m.rotx(-Math.PI/2 * 0.8);
         ...
         m.save();
            m.translate(0, -LL, 0);
            // m.rotx(1 + Math.sin(t));
            m.rotx(1-((Math.cos(T)*0.5+0.5)*0.9));
      ...
      m.save();
         ...
         for (let s = -1 ; s <= 1 ; s += 2) {
            ...
            m.save();
               ...
               //shoulder
               m.save();
                  m.rotx(-Math.PI/2*0.5);
                  m.rotx(-(Math.cos(T)*0.25 + 0.25));
                  m.rotz(Math.sin(t)*0.6 + 0.6*s);
                  ...
                  m.save();
                     ...
                     m.save();
                        m.translate(0,-AL,0);
                        m.rotx(-Math.cos(T)*0.2 + 0.2);
                        m.rotz(-Math.cos(t)*0.3 - 0.3*s);
                        m.roty(Math.PI/4 * s);
